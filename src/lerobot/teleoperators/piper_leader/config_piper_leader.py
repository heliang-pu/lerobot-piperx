...
class PiperLeaderConfigBase:
    ...
    # Some hardware only publishes leader CAN frames while the arm is moving.
    allow_missing_ctrl_mode_on_connect: bool = False
    first_action_timeout_s: float = 30.0
...
def _validate_piper_leader_config(config: PiperLeaderConfigBase) -> None:
    ...
    if not isinstance(config.allow_missing_ctrl_mode_on_connect, bool):
        raise ValueError("allow_missing_ctrl_mode_on_connect must be true or false.")
    if config.first_action_timeout_s < 0:
        raise ValueError("`first_action_timeout_s` must be >= 0.")
