...
class PiperLeader(Teleoperator):
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        ctrl_mode = guard_piper_ctrl_mode_on_connect(
            arm=self.arm,
            interface_name=self.config.port,
            allow_missing=self.config.allow_missing_ctrl_mode_on_connect,
        )
        if ctrl_mode is None:
            logger.warning(
                "[%s] could not read arm ctrl_mode on connect; continuing because "
                "allow_missing_ctrl_mode_on_connect=true. The first teleop action will wait for "
                "a live leader CAN frame before commanding the follower.",
                self.config.port,
            )
    ...
    def _read_joint_from_feedback(self) -> dict[str, float] | None:
        ...
        if getattr(joint_msg, "time_stamp", 0.0) <= 0:
            return None
    ...
    def _read_gripper_from_feedback(self) -> float | None:
        gripper_msg = self.arm.GetArmGripperMsgs()
        if getattr(gripper_msg, "time_stamp", 0.0) <= 0:
            return None
        gripper_state = getattr(gripper_msg, "gripper_state", None)
    ...
    def _read_live_raw_action(self) -> RobotAction | None:
    ...
    def _wait_for_first_raw_action(self) -> RobotAction:
        deadline = time.monotonic() + self.config.first_action_timeout_s
        while True:
            action = self._read_live_raw_action()
            ...
            if time.monotonic() >= deadline:
                raise RuntimeError(
                    f"No Piper leader action received on '{self.config.port}' within "
                    f"{self.config.first_action_timeout_s:.2f}s. Move the leader arm so it publishes "
                    "a CAN frame, or disable allow_missing_ctrl_mode_on_connect if this arm should "
                    "publish static status frames."
                )
            time.sleep(0.005)
    ...
    def _read_raw_action(self) -> RobotAction:
        ...
        if self.config.allow_missing_ctrl_mode_on_connect:
            if self._last_raw_action is None:
                return self._wait_for_first_raw_action()
            return dict(self._last_raw_action)

        return {f"{joint_name}.pos": 0.0 for joint_name in PIPER_JOINT_NAMES} | {"gripper.pos": 0.0}
